...
public final class Actor<B> {
  ...
  private record StackLocal<T>(Object stackLocal) {
    ...
    public boolean isBound() {
      if (SCOPE_LOCAL_AVAILABLE) {
        return ((ScopedValue<T>) stackLocal).isBound();
      }
    ...
    public T get() {
      if (SCOPE_LOCAL_AVAILABLE) {
        return ((ScopedValue<T>) stackLocal).get();
      }
    ...
    public static <T> StackLocal<T> newInstance() {
      if (SCOPE_LOCAL_AVAILABLE) {
        return new StackLocal<>(ScopedValue.newInstance());
      }
  ...
  private static final class SignalMessage implements Message<Object> {
    ...
    return Thread.ofVirtual().name(actor.name).start(() -> {
      StackLocal.where(CURRENT_ACTOR, actor, () -> {
        ...
        for (;;) {
          ...
          } catch(@SuppressWarnings("ErrorNotRethrown") RestartError restartError) {
            actor.mailbox.clear();
